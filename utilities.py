...
class utilities(commands.Cog):
    ...
    async def anime(self, ctx, *, specify=""):
        ...
        for g, r in reaction_dict.items():
            if g in g_value:
                r_possible.append(r)
        await discord.Message.add_reaction(msg, rd.choice(r_possible)) if r_possible else await discord.Message.add_reaction(msg, "🔥")
        
        # update_exp(ctx, ctx.author)

    @commands.command(aliases=['s'])
    async def search(self,ctx,*,name=""):
        anime = search_anime_name(name)
        if anime=="unidentify":
            error_msg = discord.Embed(title="Cannot be found",color=discord.Colour.from_rgb(0, 255, 255),)
    ...
    async def help(self, ctx):  # help function
        ...
        p1.set_thumbnail(url=rd.choice(gif_list))
        ...
        p1.set_footer(text=page + f" 1/3")

        p2 = discord.Embed(title="Commands", color=discord.Colour.green())
        p2.set_thumbnail(url=rd.choice(gif_list))
        p2.add_field(name=f"`.anime | .a [genre(s)]`", value="> Pick a random anime... \uD83D\uDCEC", inline=False)
        p2.set_footer(text=page + f" 2/3")

        p3 = discord.Embed(title="Commands", color=discord.Colour.blue())
        p3.set_thumbnail(url=rd.choice(gif_list))
        p3.add_field(name=f"`.search | .s [anime_name)]`", value="> Search your anime... 🔍", inline=False)
        p3.set_footer(text=page + f" 3/3")
